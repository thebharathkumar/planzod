...
router.post(
  ...
  async (req, res) => {
    ...
    if (ticket.status !== "issued")
      return res.json({ status: "voided" as const, ticketId, eventId });

    const usedRes = await pool.query(
      "SELECT 1 FROM checkins WHERE ticket_id = $1",
      [ticket.id],
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    if (ticket.status !== "issued")
      throw new HttpError(400, "Ticket is voided");
    ...
    if (insert.rowCount === 0)
      return res.json({ status: "already_checked_in" as const });
