...
router.post(
  ...
  async (req, res) => {
    ...
    if (salesEnd && !(salesEnd > salesStart))
      throw new HttpError(400, "salesEnd must be after salesStart");
...
router.put(
  ...
  async (req, res) => {
    ...
    const salesStartDate = body.salesStart
      ? new Date(body.salesStart)
      : new Date(current.sales_start);
    const salesEndDate =
      body.salesEnd === undefined
        ? current.sales_end
          ? new Date(current.sales_end)
          : null
        : body.salesEnd
          ? new Date(body.salesEnd)
          : null;
    const salesEndIso = salesEndDate
      ? salesEndDate.toISOString()
      : body.salesEnd === undefined
        ? current.sales_end
        : null;

    if (salesEndDate && !(salesEndDate > salesStartDate))
      throw new HttpError(400, "salesEnd must be after salesStart");

    const sold = current.total_qty - current.remaining_qty;
    if (body.totalQty < sold)
      throw new HttpError(
        400,
        "totalQty cannot be less than tickets already sold",
      );
