...
export default function OrganizerDashboardPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const org = await auth.apiFetch<{ organizer: Organizer }>(
        "/organizer/me",
      );
      setOrganizer(org.organizer);
      if (org.organizer) {
        const ev = await auth.apiFetch<{ events: OrganizerEvent[] }>(
          "/organizer/events",
        );
        setEvents(ev.events ?? []);
        const met = await auth.apiFetch<{ metrics: OrganizerMetrics }>(
          "/organizer/metrics",
        );
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link className="btn-secondary" to="/organizer/refunds">
            Refund requests
          </Link>
          <button
            type="button"
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="card">
          ...
          <div className="mt-2 font-display text-3xl font-bold text-brand-400">
            {metrics?.events_count ?? events.length}
          </div>
        ...
        <div className="card">
          ...
          <div className="mt-2 font-display text-3xl font-bold text-brand-400">
            {metrics?.tickets_sold ?? 0}
          </div>
          <div className="text-sm text-surface-500">
            Avg ${((metrics?.avg_ticket_price_cents ?? 0) / 100).toFixed(2)}
          </div>
        </div>
        <div className="card">
          <div className="text-xs font-semibold uppercase tracking-wider text-surface-500">
          ...
          <div className="text-sm text-surface-500">
            Revenue ${((metrics?.revenue_cents ?? 0) / 100).toFixed(2)}
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-surface-500">
            {metrics?.review_count ?? 0} reviews
          </span>
        </div>
      ...
      {!organizer ? (
        <div className="card">
          ...
          <p className="mt-1 text-surface-400">
            This enables event creation and check-in scanning.
          </p>
          <form
                ...
                <li key={e.id} className="card-hover">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Link
                        className="btn-secondary"
                        to={`/organizer/events/${e.id}/announcements`}
                      >
                        Send update
                      </Link>
                      {e.status === "published" ? (
                        <>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              if (
                                !confirm(
                                  "Unpublish this event? It won't be visible to attendees.",
                                )
                              )
                                return;
                              try {
                                await auth.apiFetch(
                                  `/events/${e.id}/unpublish`,
                                  { method: "POST" },
                                );
                                await load();
                              } catch (err: any) {
