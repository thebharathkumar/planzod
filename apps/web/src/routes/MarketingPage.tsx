...
const FEATURES = [
  {
    ...
    description:
      "Purpose‑built for skill‑based events with a faster path from idea to tickets.",
  },
  {
    title: "AI‑assisted creation",
    description:
      "Generate event copy, agendas, tags, and tier ideas in seconds.",
  },
  {
    title: "Payments + tickets",
    description:
      "Stripe Checkout, QR tickets, and organizer check‑in for real‑world workshops.",
  },
];
...
export default function MarketingPage() {
  return (
    <div className="space-y-16">
      <section className="card overflow-hidden bg-gradient-to-br from-surface-900 via-surface-900 to-surface-950">
        <div className="max-w-2xl space-y-5">
          ...
          <p className="text-lg text-surface-400">
            Map‑first discovery, AI‑assisted creation, and instant ticketing —
            built for local workshops, classes, and training sessions.
          </p>
      ...
      <section>
        <h2 className="font-display text-2xl font-semibold text-surface-50">
          Why teams choose Planzo
        </h2>
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {FEATURES.map((f) => (
            <div key={f.title} className="card-hover">
              <h3 className="font-display text-lg font-semibold text-surface-50">
                {f.title}
              </h3>
      ...
      <section className="card">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          {STEPS.map((s, idx) => (
            <div
              key={s.title}
              className="rounded-xl border border-surface-700 bg-surface-950/50 p-5"
            >
